...
class DateTimePickerWindowClass():
    def __init__(self,vars:'variablesclass',dttype:str,initial_date=None,initial_time=None):
        self.type = dttype
        self.vars = vars
        ...
        if initial_date:
            ...
            self.day_picker = tkcalendar.Calendar(self.calendar_frame,selectmode='day',year=int('20{}'.format(year)),month=int(month),day=int(day))
        else:self.day_picker = tkcalendar.Calendar(self.calendar_frame,selectmode='day')
...
def process_entries(self,entry_list:list[str]):
    index = 0
    print('process_entries: {} entries'.format(len(entry_list)))
    ...
    def pull_data_from_folder(self,start_date=None,end_date=None):
        ...
        for file in files:
            ...
            try:
                ...
                continue
            if filedatetime >= self.__start_date and filedatetime <= self.__end_date:
                print('adding file {} to fetch'.format(filepath))
                files_to_fetch.append(filepath)
        file_count = 1
        for file in files_to_fetch:
            self.__update_status('Reading DebugLog file {} of {}: {}'.format(file_count,len(files_to_fetch),file))
            ...
            self.__all_data += remote_file.readlines()[1:]
            print('after file {}, total lines {}'.format(file_count,len(self.__all_data)))
            file_count += 1
        ...
        while cur_end <= len(self.__all_data):
            if len(self.__threads) < 11:
                ...
                for thread in self.__threads:
                    thread.join()
                self.__threads = []
            if cur_start >= len(self.__all_data):
                break
        for thread in self.__threads:
            thread.join()
        self.__threads = []
        time.sleep(1)
        print('total items:{}'.format(len(self.__all_data)))
